...
std::variant <Date, int, float, std::string> RightFunction::call(std::vector <Row>& table_data, const std::vector <std::string>& table_datatypes)
{
    ...
    int amount = std::stoi(arguments[2]);

    // is_valid
    if(!Validator::has_enough_arguments(arguments, ExpectedArguments)){
        std::cout << "Wrong amount of arguments used in RIGHT function";
        return output;
    }
    ...
    if (!Validator::is_start_index_greater_than_0(amount)){
        std::cout << "Third argument cannot be smaller than 1";
        return output;
    }
    ...
    if (holds_alternative<std::string >(exact_row.get(column_index))) {
        std::string found_word = get<std::string>(exact_row.get(column_index));
        if (amount >= found_word.size()){
            return found_word;
        }
        int word_len = static_cast<int>(found_word.length());
        for (int i = word_len - amount; i < word_len; i++){
            output_string += found_word[i];
        }
