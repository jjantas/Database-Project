...
std::variant <Date, int, float, std::string> MeanFunction::call(std::vector <Row>& table_data, const std::vector <std::string>& table_datatypes) {
    ...
    if(!Validator::has_enough_arguments(arguments, ExpectedArguments)){
        std::cout << "Wrong amount of arguments used in MEAN function";
        return output;
    }
    ...
    int amount_of_rows = end_index - start_index + 1;
    float_output /= float(amount_of_rows);
    output = float_output;
    return output;
}
