...
std::variant <Date, int, float, std::string> DayFunction::call(std::vector <Row>& table_data, const std::vector <std::string>& table_datatypes)
{
    ...
    if(!Validator::has_enough_arguments(arguments, ExpectedArguments)){
        std::cout << "Wrong amount of arguments used in Day function";
        return output;
    }
