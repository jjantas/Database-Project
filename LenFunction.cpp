...
std::variant <Date, int, float, std::string> LenFunction::call(std::vector <Row>& table_data, const std::vector <std::string>& table_datatypes)
{
    ...
    if(!Validator::has_enough_arguments(arguments, ExpectedArguments)){
        std::cout << "Wrong amount of arguments used in LEN function";
        return output;
    }
    ...
    if (holds_alternative<std::string >(exact_row.get(column_index))) {
        ...
        return static_cast<int>(found_word.length());
    }
    return -1;
}
