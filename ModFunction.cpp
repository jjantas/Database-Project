...
std::variant <Date, int, float, std::string> ModFunction::call(std::vector <Row>& table_data, const std::vector <std::string>& table_datatypes) {
    ...
    if(!Validator::has_enough_arguments(arguments, ExpectedArguments)){
        std::cout << "Wrong amount of arguments used in MOD function";
        return output;
    }
    if(!Validator::is_convertable_to_float(arguments[2])){
        std::cout << "Divider cant be converted to float";
        return output;
    }
    int column_index = Decomposer::column_to_int(arguments[0]);
    int row_index= std::stoi(arguments[1]);
    float divider = std::stof(arguments[2]);
    ...
    std::variant <Date, int, float, std::string> exact_element = exact_row.get(column_index);
    if (holds_alternative<int>(exact_element)) {
        int int_output = get<int>(exact_element);
        output =  std::fmod(float(int_output), divider);
    }
    else if (holds_alternative<float>(exact_element)) {
        float float_output = get<float>(exact_element);
        output =  std::fmod(float_output, divider);
    }
    return output;
}
